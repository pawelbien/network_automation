# network_automation/platforms/cisco_xr/info.py
...
_VERSION_RE = re.compile(r"Cisco IOS XR Software, Version (\S+)")
_MODEL_RE = re.compile(r"[Cc]isco (.+?) \(")
...
def _parse_show_version(output: str) -> dict:
    ...
    return {
        ...
        "version": version.group(1).split("[")[0],
        "model": model.group(1),
    }


def _parse_inventory(output: str) -> str:
    serial = _SERIAL_RE.search(output)
    if not serial:
        raise ValueError("Serial number not found in 'show inventory' output")
...
def get_info(client):
    ...
    version_output = client.conn.send_command("show version")
    parsed = _parse_show_version(version_output)

    inventory_output = client.conn.send_command("show inventory")
    serial = _parse_inventory(inventory_output)
