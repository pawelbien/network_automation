# network_automation/platforms/opnsense/debug_log.py

"""
Opt-in DEBUG-level diagnostics for OPNsense operations.
...
Helpers (firmware.py etc.) are tested with minimal fake clients exposing
only .conn/.logger, with no .context — so these functions look up
.context defensively (getattr, default False) rather than requiring it,
preserving the "helpers need no lifecycle" contract instead of forcing
every caller to grow a .context attribute.
"""
